...
data TwilioRequest a
  = TwilioRequest (Req.BasicRequest a)
...
requestedUrl req = "https://" <> host <> "/" <> path
  where
  host = fromMaybe "" $ Req.lookupHeader "Host" req
...
formToObject formData = foldr insertInObject Object.empty (toArray formData)
  where
  insertInObject (Tuple k (Just v)) o = Object.insert k v o
...
wrapTwilioAuth ::
  ...
  Aff res
wrapTwilioAuth config authErrorResponse router req =
