...
executeMigration pool id query = runExceptT do
  liftEffect $ Console.log $ "Running migration: " <> show id
  ExceptT $ showDBError <$> runQuery pool \conn -> do
...
currentIntVersion pool = runExceptT do
  ExceptT $ showDBError <$> runQuery pool \conn -> do
    PG.execute conn (PG.Query createTableQuery) Row0
  rows <- ExceptT $ showDBError <$> runQuery pool \conn -> do
...
intVersionStore pool = { currentVersion: currentIntVersion pool
