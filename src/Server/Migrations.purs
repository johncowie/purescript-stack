...
runMigration {versionStore, executor} migration = runExceptT do
...
runRollback {versionStore, executor} migration = runExceptT do
...
migrate migrator@{versionStore, migrationStore, logger} = runExceptT do
...
rollback migrator@{versionStore, migrationStore} = runExceptT do
  migrations <- ExceptT migrationStore.loadMigrations
  currentVersion <- ExceptT versionStore.currentVersion
  let previousVersion = previousMigration currentVersion migrations
