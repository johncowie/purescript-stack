...
showBotMessage (RequestContactFreq fn ln) =
  Str.joinWith "\n"
...
showBotMessage (FriendsList friends) = "*Your friends:* \n\n" <> (Str.joinWith "\n" $ map friendName friends)
  where
  friendName f = case L.view Friend._lastSeen f of
    Nothing -> "_" <> (show $ L.view Friend._name f) <> "_"
    _ -> show $ L.view Friend._name f

showBotMessage (NextToSee []) =
  unlines
...
showBotMessage (NextToSee friendAndOverdueTuples) = unlines $ map row friendAndOverdueTuples
  where
  row (friend /\ overdueDays) =
    let
      friendName = show $ L.view Friend._name friend

      durationStr = showDuration (daysToInt overdueDays)
    in
      friendName <> " (" <> durationStr <> " overdue)"

showBotMessage (JustSeenFriend friend) = "Thanks, I've noted that you've just been in touch with *" <> friendName <> "*."
  where
  friendName = show $ L.view Friend._name friend
...
showBotMessage (UpdateFriendMenu friendName) =
  Str.joinWith "\n"
...
storeEvents db waNumber events =
  runExceptT do
...
isCommand cmd input = if (cleaned == cmd) then Just unit else Nothing
  where
  cleaned = Str.toLower $ Str.trim $ stripPunctuation input
...
nameMatch input (id /\ friend) = case tokens input of
  ...
  _ -> false
  where
  firstNameToken = Str.toLower $ L.view (Friend._name >>> FN._firstName) friend
...
requestContactFreq firstName lastName = (AwaitingContactFreq { firstName, lastName }) /\ [ RequestContactFreq firstName lastName ]
...
saveFriend deps number firstName lastName freq = do
  ...
  let
    id = fromMaybe (wrap 0) $ maximum $ map fst $ St.friendList st
...
listFriends deps number = do
  ...
  let
    friends = sortWith (L.view Friend._name >>> show) $ map snd $ St.friendList state
...
listNextToSee st = do
  ...
  let
    nextToSee = take 10 $ St.overdueContacts n st
...
markFriendSeenDate deps number id friend successMsg dt = do
  ...
  if (dt <= n) then do
    ...
    pure $ Idle
      /\ [ successMsg dt friend
        , MainMenu
        ]
  else
    pure $ AwaitingLastSeenDate { id, friend } /\ [ DateInFuture, RequestDate ]
...
deleteFriend deps number id friend = do
  ...
  pure $ Idle
    /\ [ DeletedFriend $ show $ L.view Friend._name friend
      , MainMenu
      ]
...
handleUserMessage deps number New st = tryTo (liftF ignoreMsg) (liftF (const welcomeUser))
...
handleUserMessage deps number AwaitingFirstName st = tryTo (liftF anyMsg) (liftF requestLastName)

handleUserMessage deps number (AwaitingLastName { firstName }) st = tryTo (liftF anyMsg) (liftF (requestContactFreq firstName))

handleUserMessage deps number (AwaitingContactFreq { firstName, lastName }) st = tryTo (liftF (tokens >>> parseDuration)) (saveFriend deps number firstName lastName)

handleUserMessage deps number (AwaitingLastSeenDate { id, friend }) st = tryTo (liftF parseDate) (markFriendSeenDate deps number id friend SawFriendOnDate)

processMessage :: Deps -> WhatsAppNumber -> ChatState -> String -> Aff (Either String BotResult)
processMessage deps number chatState msg =
  runExceptT do
    state <- ExceptT $ loadState deps.db number
...
inMemoryChatBotStore newChatState = do
  ...
  pure
    { storeState: \id st -> liftEffect $ map Right $ Ref.modify_ (M.insert id st) ref
    , retrieveState:
        \id ->
          liftEffect $ map Right
            $ do
                m <- Ref.read ref
