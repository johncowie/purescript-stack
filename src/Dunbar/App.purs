...
renderFriendRow now (Tuple id friend) =
  ...
  where
  timeElapsedStr = maybe "-" UDT.timeElapsedStr $ Friend.timeSinceLastSeen now friend
...
renderFriendsList model =
  ...
  where
  sortF (Tuple id friend) = L.view Friend._name friend
...
renderFriendForm model =
  H.div []
    [ Input.renderStringInput UpdateInput firstNameInput "first name" model
...
renderUpdateFriendForm id model friend =
  renderSection title
    $ H.div []
        [ H.div [] [ Input.renderDropdown UpdateInput (contactFreqInput id) contactFrequencies model ]
        , H.div []
            [ H.text "Birthday: "
            , Input.renderStringInput UpdateInput (birthdayInput id) "birthday" model
            ] -- TODO
  ...
  where
  title = "Update '" <> show (L.view Friend._name friend) <> "'"
...
update model (UpdateFriend id) =
  either (alertError model)
  ...
  where
  update1 =
    State.updateDesiredContactFrequencyEvent id
      <$> Input.parseStringInput (contactFreqInput id) model

  update2 =
    State.updateNotesEvent id
      <$> Input.parseStringInput (notesInput id) model

  update3 =
    State.updateBirthdayEvent id
      <$> Input.parseStringInput (birthdayInput id) model

  clearedInputs =
    Input.clearInput (contactFreqInput id)
      $ Input.clearInput (notesInput id)
      $ Input.clearInput (birthdayInput id)
      $ model
...
update model (JustSeen id (Just timestamp)) = { effects: [], model, events: [ event ] }
  where
  event = State.justSeenEvent id timestamp
