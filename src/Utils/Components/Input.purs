...
data InputSetter model
  = InputSetter (model :-> String)
...
stringInput_ ::
  ...
  a ->
  StringInput model a
stringInput_ _inputs inputId initialVal =
  Input
    { validator: parseInput
    , lens: _inputs >>> L._mapVal startData inputId -- TODO remove?
    }
  where
  startData = emptyInputData { rawValue = showInput initialVal }
...
renderStringInput actionF (Input input) placeholder model =
  ...
  where
  inputData = L.view input.lens model
...
renderDropdown ::
  ...
  model ->
  H.Html msg
renderDropdown actionF (Input input) options model = H.select [ E.onValueChange (E.always (actionF (mkInputSetter (Input input)))) ] $ map (renderOption selected) options
  where
  selected = either (const Nothing) Just $ input.validator currentValue
...
renderDropdown_ ::
  ...
  model ->
  H.Html msg
renderDropdown_ actionF input optionVals model = renderDropdown actionF input options model
  where
  options = map (\v -> Tuple (showInput v) v) optionVals
