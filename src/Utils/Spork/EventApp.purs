...
data MsgResult st ev msg
  = Internal (InternalMsg st ev)
  | External msg
...
mkSubs eventApp model = maybe (App.batch []) App.lift tickSubM
  where
  tickSubM = case eventApp.tick of
...
loadEvents eventStore snapshotStore =
  alertErrorShow
    <$> runExceptT do
        snapshotM <- ExceptT snapshotStore.retrieveLatestSnapshot
        let
          stateM = _.state <$> snapshotM
        let
          lastEventId = fromMaybe 0 $ _.upToEvent <$> snapshotM
...
applyEvents reducer snapshotStateM events state = foldr reducer initialState events
  where
  initialState = fromMaybe state snapshotStateM
...
saveEvent eventStore snapshotStore reducer state event = do
  ...
  let
    updatedState = reducer event state
  if eventId `mod` 10 == 0 then
...
internalUpdate ::
  forall st ev model msg.
  App st ev model msg ->
  model ->
  InternalMsg st ev ->
  App.Transition Aff model (MsgResult st ev msg)
internalUpdate { _state, reducer, eventStore, snapshotStore } model (SaveEvents events) = { effects: App.batch [ effect ], model }
  where
  effect =
    alertErrorShow
      <$> runExceptT do
...
internalUpdate { _state, reducer } model (ProcessEvents stateM events) = App.purely updatedModel
  where
  updatedModel = L.over _state (applyEvents reducer stateM events) model

internalUpdate eventApp model LoadEvents = { effects: App.batch [ load ], model }
  where
  load = loadEvents eventApp.eventStore eventApp.snapshotStore

internalUpdate { _state } model (ResetState s) = App.purely $ L.set _state s model
...
externalUpdate { _state, reducer, eventStore, update } m msg = { effects: allEvents, model }
  where
  { effects, model, events } = update m msg
...
makeWithSelector eventApp selector = App.makeWithSelector interpreter app selector
  where
  interpreter = basicAff affErrorHandler `merge` (runTicker tickSecs)
